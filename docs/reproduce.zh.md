...
| R35 | 重新构建时 `prepare:runtime` 报 `TypeError: fetch failed`（`@electron/get` 下载 Electron 失败） | `prepare:runtime` 每轮都重新解压 Electron，取件走 `@electron/get` → 默认直连 `github.com/electron/electron/releases`，本机直连不通；而 `@electron/get` 的缓存目录名与**取件 URL** 绑定（`BUILD_PATHS.downloads` 里那份 9/14 下好的 `electron-v44.0.0-win32-x64.zip` 因此不会被复用），所以「缓存里有 zip」不等于不重新下载。**对策**：构建时把镜像或代理带上——`ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/`，或按本机 Clash 的 `HTTP_PROXY`/`HTTPS_PROXY=http://127.0.0.1:7897`（实测两者都能让这一阶段过）。这与 `DSH_LOCAL_NPM_REGISTRY`（给 pnpm 用）是两回事 |
